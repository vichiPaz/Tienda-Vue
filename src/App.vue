...
<template>
  <div class="app-root d-flex flex-column min-vh-100">
    ...
    <nav class="navbar navbar-dark bg-dark px-3">
      <span class="navbar-brand">Mi Tienda</span>
      ...
      <div v-if="state.user" class="d-flex align-items-center gap-3">
        ...
        <button class="btn btn-outline-light btn-sm" @click="actions.logout()">Cerrar sesión</button>
      </div>

      <div v-else class="d-flex gap-2">
        <button class="btn btn-outline-light btn-sm" @click="goLogin">Iniciar sesión</button>
        <button class="btn btn-primary btn-sm" @click="goRegister">Crear cuenta</button>
      </div>
    </nav>
    ...
    <div class="container py-4 flex-grow-1">
      ...
      <div v-if="!state.user" class="row justify-content-center">
        <div class="col-12 col-md-6 col-lg-5">
          <LoginForm v-if="authView === 'login'" @switch-to-register="authView='register'" />
      ...
      <template v-else>
        ...
        <div v-if="state.view === 'home'" class="row mb-3">
          <div class="col-12 col-md-6 col-lg-4">
            <SearchBox v-model="state.search" placeholder="Buscar productos..." />
          </div>
        </div>
...
.navbar { border-bottom: 1px solid #1f2937; }
a { color: #60a5fa; }
a:hover { color: #93c5fd; }

.app-root { background: #0f172a; }
